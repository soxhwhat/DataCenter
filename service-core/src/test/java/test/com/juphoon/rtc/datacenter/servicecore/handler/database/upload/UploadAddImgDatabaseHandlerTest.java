...
public class UploadAddImgDatabaseHandlerTest {

    @InjectMocks
    private UploadAddImgDatabaseHandler uploadAddImgDatabaseHandler;

    @Test
    public void testCareEvents() {
        List<EventType> events = uploadAddImgDatabaseHandler.careEvents();
        Assert.notEmpty(events, "careEvent为空");
        Assert.isTrue(events.contains(INSERT_SUCCESS_IMAGE_PO), INSERT_SUCCESS_IMAGE_PO.name());
    }
}
