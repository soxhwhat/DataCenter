...
public abstract class StatusCollectionServiceServer extends Common.ObjectServer {
    public abstract void putStatus_begin(Common.ServerCall __call,DataCollection.FlowStatus event) throws Common.Exception;
    public abstract void endStatus_begin(Common.ServerCall __call,DataCollection.FlowStatus event) throws Common.Exception;
    ...
    private void __cmd_putStatus(Common.ServerCall __call,Common.IputStream __iput) throws Common.Exception {
        __outer: {
            Common.VerList __vers = __call.verList();
            DataCollection.FlowStatus event;
            switch (__vers.ver(false)) {
            case 0:
                event = DataCollection.FlowStatus.__read(__iput);
                break;
            default: break __outer;
            }
            __start(false);
            putStatus_begin(__call,event);
            return;
        }
        Common.OputStream __oput = new Common.OputStream();
