...
public class TestEventProcessorWithEmptyLastHandler extends AbstractEventProcessor {
    @Autowired
    ...
    public ProcessorId processorId() {
        return ProcessorId.TEST_EVENT_WITH_EMPTY_LAST_HANDLER;
    }
    ...
    public LastInnerHandler<EventContext> lastInnerEventHandler() {
        return new MyLastInnerHandler(this);
    }


    public static class MyLastInnerHandler extends LastInnerHandler<EventContext> {
        public MyLastInnerHandler(IProcessor<EventContext> processor) {
            super(processor);
        }
